...
export const dlCategories: DLCategory[] = [
  {
    id: "foundations",
    title: "From-Scratch Foundations",
    description: "The math and machinery underneath every model below: gradients, optimizers, and data pipelines, implemented and benchmarked by hand.",
  },
  {
    id: "cnns",
    title: "Convolutional Networks",
    description: "Designing CNN architectures and the datasets that train them, then ablating both to see what actually moves accuracy.",
  },
  {
    id: "detection-segmentation",
    title: "Detection & Segmentation",
    description: "Going beyond whole-image labels: localizing and delineating objects within a scene.",
  },
  {
    id: "generative",
    title: "Generative Models",
    description: "Learning to synthesize realistic images from noise.",
  },
  {
    id: "sequence",
    title: "Sequence Models",
    description: "Modeling and generating sequences with attention.",
  },
];
...
export const dlProjects: DLProject[] = [
  {
    ...
    intro: [
      "The plumbing underneath every project below. A small class hierarchy (growth/decay models over a biological sequence) exercises Python's iterator protocol (__iter__/__next__), operator overloading (__eq__), and callable objects (__call__), the OOP patterns PyTorch itself is built on. Then the part that matters for everything after it: a custom torch.utils.data.Dataset over a personally-photographed set of 20 household objects, a torchvision augmentation pipeline (flip, rotation, resized crop), and a systematic benchmark of DataLoader's batch_size × num_workers space.",
    ],
    sections: [
      {
    ...
    writeups: [
      { label: "Python foundations, in depth (PDF)", href: `${ASSET}/reports/hw01-python-oop.pdf` },
      { label: "Data pipelines, in depth (PDF)", href: `${ASSET}/reports/hw02-datasets-dataloaders.pdf` },
    ],
  },
  {
    ...
    intro: [
      "Built on ComputationalGraphPrimer, a tiny computational graph library with no autograd: every gradient here is one derived and coded by hand. It's subclassed three ways (plain SGD, SGD with momentum, and Adam) for both a single artificial neuron and a small multi-neuron network, then benchmarked against the same architectures built in torch.nn and trained with torch.optim.",
    ...
    writeups: [{ label: "Full write-up (PDF)", href: `${ASSET}/reports/hw03-backprop-optimizers.pdf` }],
  },
  {
    ...
    writeups: [{ label: "Full write-up (PDF)", href: `${ASSET}/reports/hw04-cnn-dataset-design.pdf` }],
  },
  {
    ...
    intro: [
      "Two related studies on CIFAR-10, both built on the DLStudio training framework. The first trains three networks of increasing depth (2, 3, and 8 convolutional layers, the deepest with only two pooling stages to preserve spatial resolution) to see where added depth actually pays off. The second isolates one design choice inside a ResNet-style SkipBlock: does downsampling by a stride-2 1×1 convolution behave differently than downsampling by max-pooling?",
    ],
    sections: [
      {
...
out = self.downsampler2(out)`,
    ...
    writeups: [
      { label: "Depth ablation, in depth (PDF)", href: `${ASSET}/reports/hw05-cnn-depth.pdf` },
      { label: "Skip connections, in depth (PDF)", href: `${ASSET}/reports/hw06-skip-connections.pdf` },
    ],
  },
  {
    ...
    writeups: [{ label: "Full write-up (PDF)", href: `${ASSET}/reports/hw07-object-detection.pdf` }],
  },
  {
    ...
    writeups: [{ label: "Full write-up (PDF)", href: `${ASSET}/reports/hw08-semantic-segmentation.pdf` }],
  },
  {
    slug: "gan-diffusion",
    ...
    writeups: [{ label: "Full write-up (PDF)", href: `${ASSET}/reports/hw09-gan-diffusion.pdf` }],
  },
  {
    ...
    writeups: [{ label: "Full write-up (PDF)", href: `${ASSET}/reports/hw10-transformer-translation.pdf` }],
  },
];
